...
Options:
 ...
 -t <threads> --threads=<threads>        Number of threads to run in parallel [default: )" AS_STR(
        DEFAULT_THREADS) R"(]
...
std::string loadCustomTemplate(std::string customIndexPath)
{
  customIndexPath = kiwix::isRelativePath(customIndexPath)
                        ? kiwix::computeAbsolutePath(
                            kiwix::getCurrentDirectory(), customIndexPath)
                        : customIndexPath;
  if (!kiwix::fileReadable(customIndexPath)) {
    throw std::runtime_error("No such file exist (or file is not readable) "
                             + customIndexPath);
  }
  if (kiwix::getMimeTypeForFile(customIndexPath) != "text/html") {
    throw std::runtime_error("Invalid File Mime Type "
                             + kiwix::getMimeTypeForFile(customIndexPath));
  }
...
#define FLAG(NAME, VAR)        \
  if (arg.first == NAME) {     \
    VAR = arg.second.asBool(); \
    continue;                  \
  }
#define STRING(NAME, VAR)                           \
  if (arg.first == NAME && arg.second.isString()) { \
    VAR = arg.second.asString();                    \
    continue;                                       \
  }
#define STRING_LIST(NAME, VAR, ERRORSTR) \
  if (arg.first == NAME) {               \
    if (arg.second.isStringList()) {     \
      VAR = arg.second.asStringList();   \
      continue;                          \
    } else {                             \
...
#define INT(NAME, VAR, ERRORSTR) \
  if (arg.first == NAME) {       \
    if (isLong(arg.second)) {    \
      VAR = arg.second.asLong(); \
      continue;                  \
    } else {                     \
      errorString = ERRORSTR;    \
      break;                     \
...
void listenDirectoryChanges(
    const std::vector<std::string>& paths,
    kiwix::Manager& manager,
    std::shared_ptr<kiwix::UpdatableNameMapper>& nameMapper)
{
  ...
  for (it = paths.begin(); it != paths.end(); ++it) {
    int wd = inotify_add_watch(
        inotifyFd, it->c_str(), IN_MODIFY | IN_CREATE | IN_DELETE);
    if (wd == -1) {
      std::cerr << "Error adding watch for: " << *it << std::endl;
    } else {
  ...
  if (watchDescriptors.empty()) {
    std::cerr << "No valid directories to monitor. Exiting...\n";
    close(inotifyFd);
    return;
  }
  ...
  while (true) {
    ...
    if (length == -1) {
      ...
      if (!reloadLibrary(manager, paths)) {
        exit(1);
      }
      nameMapper->update();
    }
  ...
  std::vector<int>::iterator wdIt;
  for (wdIt = watchDescriptors.begin(); wdIt != watchDescriptors.end();
       ++wdIt) {
    close(*wdIt);
  }
  close(inotifyFd);
...
int main(int argc, char** argv)
{
  ...
  for (auto const& arg : args) {
    ...
    INT("--ipConnectionLimit",
        ipConnectionLimit,
        "IP connection limit must be an integer")
  ...
  if (!libraryPath.empty()) {
    ...
    for (it = zimPathes.begin(); it != zimPathes.end(); it++) {
      if (!manager.addBookFromPath(*it, *it, "", false)) {
        if (skipInvalid) {
          std::cerr << "Skipping invalid '" << *it << "' ...continuing"
                    << std::endl;
        } else {
  ...
  auto nameMapper = std::make_shared<kiwix::UpdatableNameMapper>(
      library, noDateAliasesFlag);
  ...
  if (!addresses.addr6.empty())
    std::cout << "  - " << prefix << "[" << addresses.addr6 << "]" << suffix
              << std::endl;
  if (monitorLibrary) {
#ifdef __linux__
    std::cout << "monitorThread Started" << std::endl;
    std::thread monitorThread(
        [&]() { listenDirectoryChanges(libraryPaths, manager, nameMapper); });
    monitorThread.detach();  // Runs indefinitely (modify for controlled exit)
#endif
  }
